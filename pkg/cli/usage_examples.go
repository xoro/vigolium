...
var scanExamples = FormatExamples(
	...
	"# Pipe a raw HTTP / Burp request from stdin (auto-detected; cookies & headers preserved)",
	"cat request.txt | vigolium scan",
	"# Same, but skip the known-issue (Nuclei) phase and use a throwaway database",
	"cat request.txt | vigolium scan --skip kis --stateless",
	"",
...
var trafficExamples = FormatExamples(
	...
	"vigolium traffic",
	"# List EVERY stored record (lift the -n/--limit cap)",
	"vigolium traffic --all",
	"# Page through results",
	"vigolium traffic -n 50 --offset 100",
	"# Tree view (host/path hierarchy)",
	"vigolium traffic tree",
	"vigolium traffic --tree",
	"",
	"# --- Search ---",
	"# Fuzzy search across URLs, paths, and hosts (positional term)",
	"vigolium traffic admin",
	`vigolium traffic "api/v2"`,
	"# Same fuzzy search via the --search flag",
	`vigolium traffic --search "/api/users"`,
	"# Search within header names and values (e.g. find Authorization headers)",
	`vigolium traffic --header "Authorization"`,
	`vigolium traffic --header "X-Api-Key"`,
	"# Search within request/response body content",
	`vigolium traffic --body "password"`,
	`vigolium traffic --body "BEGIN RSA PRIVATE KEY"`,
	"",
	"# --- Filter ---",
	"# Filter by host pattern (wildcard supported)",
	`vigolium traffic --host "*.example.com"`,
	"# Filter by an exact hostname",
	"vigolium traffic --host api.example.com",
	...
	"# Filter by record source (scanner, ingest-cli, ingest-proxy, seed, ...)",
	"vigolium traffic --source ingest-proxy",
	"# Filter by date range",
	"vigolium traffic --from 2026-01-01 --to 2026-02-01",
	"# Combine filters: POST requests to the API that leaked a token",
	`vigolium traffic --host api.example.com --method POST --body "token" --status 200`,
	"# Sort oldest-first by response time",
	"vigolium traffic --sort time --asc",
	"",
	"# --- Display ---",
	"# Display in Burp Suite-style format (colored request/response)",
	"vigolium traffic --burp --host example.com -n 5",
	`vigolium traffic --burp --header "Authorization"`,
	"# Show full raw HTTP request/response",
	"vigolium traffic --raw --host example.com -n 5",
	"# Select specific columns",
	"vigolium traffic --columns host,method,path,status,size",
	"# Add the auth/header columns to inspect credentials",
	"vigolium traffic --columns method,path,status,auth,req_headers",
	"# Exclude columns from the default set",
	...
	"# Replay search hits through Burp at low concurrency",
...
var findingExamples = FormatExamples(
	...
	"# High+critical only (minimum severity), filtered to a URL path",
	"vigolium finding --min-severity high --path /api",
	"# Narrow to one module type or finding source",
	"vigolium finding --module-type active",
	"vigolium finding --finding-source dynamic-assessment",
	"# Every finding from a single agentic scan (autopilot/swarm/audit run)",
	"vigolium finding --agentic-scan <uuid>",
	"# Show raw request/response for matched findings",
	"vigolium finding --raw -n 5",
	"# Burp-style colored request/response",
	"vigolium finding xss --burp",
	"# Render matches as Markdown (evidence + http blocks) to stdout",
	"vigolium finding xss --markdown > findings.md",
	"# JSONL output",
	"vigolium finding -j",
	"# JSON for a coding agent, with linked HTTP records embedded",
	"vigolium finding --min-severity medium --json --with-records",
	"# Read from a standalone export, project scoping off",
	"vigolium finding -S --db scan-results.sqlite",
	"# Pick columns and cap the result count",
	"vigolium finding --columns ID,SEVERITY,MODULE,MATCHED_AT -n 20",
)
...
var scanURLExamples = FormatExamples(
	...
	"# Write jsonl + HTML reports to files (-t is an alias for the positional URL)",
	"vigolium scan-url -t https://example.com/api/users?id=1 --format jsonl,html -o report",
	"# One-off scan into a throwaway DB, persisting only the report files",
	"vigolium scan-url -S --format jsonl -o out.jsonl https://example.com/api/users?id=1",
)
