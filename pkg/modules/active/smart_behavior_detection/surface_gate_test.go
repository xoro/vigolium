...
// TestScanPerInsertionPoint_LargeHTMLSurfaceSkipped covers the behavioral-diff
// surface gate: a large rendered HTML baseline is an unreliable break-vs-escape
...
// The existing positive test (no captured response → gate fails open) is the
// control proving the gate doesn't break normal operation.
func TestScanPerInsertionPoint_LargeHTMLSurfaceSkipped(t *testing.T) {
	...
	assert.Empty(t, res, "a large HTML content page is an unreliable behavioral-diff surface and must be skipped")
