...
// Module implements the Spring Boot Admin Exposure active scanner.
type Module struct {
	modkit.BaseActiveModule
	ds dedup.Lazy[dedup.DiskSet]
}

// New creates a new Spring Boot Admin Exposure module.
func New() *Module {
	m := &Module{
		...
		ds: dedup.LazyDiskSet("spring_boot_admin_exposure"),
	}
...
// ScanPerRequest probes the host for exposed Spring Boot Admin dashboards.
...
func (m *Module) fingerprint404(
	...
	randomPath := "/vigolium-sba-404-" + utils.RandomString(8)
...
func (m *Module) probeEndpoint(
	...
	return &output.ResultEvent{
		...
		Info: output.Info{
			Name:        fmt.Sprintf("Spring Boot Admin: %s", p.name),
			Description: p.desc,
			Severity:    p.sev,
