...
// Module implements the Spring Cloud Config Exposure active scanner.
type Module struct {
	modkit.BaseActiveModule
	ds dedup.Lazy[dedup.DiskSet]
}

// New creates a new Spring Cloud Config Exposure module.
func New() *Module {
	m := &Module{
		...
		ds: dedup.LazyDiskSet("spring_cloud_config_exposure"),
	}
...
// ScanPerRequest probes the host for exposed Spring Cloud Config Server endpoints.
...
func (m *Module) fingerprint404(
	...
	randomPath := "/vigolium-config-404-" + utils.RandomString(8)
...
func (m *Module) probeEndpoint(
	...
	return &output.ResultEvent{
		...
		Info: output.Info{
			Name:        fmt.Sprintf("Spring Cloud Config: %s", p.name),
			Description: p.desc,
			Severity:    p.sev,
			Confidence:  severity.Firm,
			Tags:        []string{"spring", "java", "config-server", "secret-disclosure"},
			Reference:   []string{"https://docs.spring.io/spring-cloud-config/docs/current/reference/html/"},
		},
	}
}
